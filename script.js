...
async function inicializarBaseDeDados() {
    try {
        ...
        if (docSnap.exists()) {
            window.baseDeDadosContas = docSnap.data();
        } else {
...
window.navTo = function(screenId) {
    ...
    document.getElementById('btnBack').style.display = (screenId !== 'screen-home') ? 'flex' : 'none';
    ...
    if(tituloHtml) tituloHtml.innerText = screenId === 'screen-sales' ? servicoSelecionado : titles[screenId];

    if(screenId === 'screen-records' || screenId === 'screen-home') {
        carregarRegistosFirebase();
    }
...
window.selecionarServico = async function(nome) {
    ...
    // Atualizar Pacotes Dinamicamente
    const pacotesNetflix = [
        { nome: 'Diário', preco: 29, dias: 1 },
        { nome: 'Semanal', preco: 99, dias: 7 },
        { nome: 'Mensal', preco: 249, dias: 30 }
    ];
    const pacotesPrime = [
        { nome: '1 Dia', preco: 35, dias: 1 },
        { nome: '7 Dias', preco: 99, dias: 7 },
        { nome: '15 Dias', preco: 150, dias: 15 },
        { nome: '30 Dias', preco: 250, dias: 30 },
        { nome: '60 Dias', preco: 480, dias: 60 }
    ];
    
    const pacotesRenderizar = nome === 'Prime Video' ? pacotesPrime : pacotesNetflix;
    let pacotesHTML = '';
    ...
    container.innerHTML = `<div style="width:100%; text-align:center; padding:20px; color:var(--text-muted);"><i data-lucide="loader" class="lucide-spin"></i> A calcular vagas disponíveis...</div>`;
...
window.salvarVendaFirebase = async function() {
    ...
    try {
        ...
        const vendaTemp = {
            id: docRef.id, nomeCliente: nome, telefone: telefoneFinal,
...
window.carregarRegistosFirebase = async function() {
    ...
    container.innerHTML = `<div class="loader-container"><i data-lucide="loader" class="lucide-spin"></i> A sincronizar dados...</div>`;
    lucide.createIcons();
    try {
...
window.calcularEstatisticas = function() {
    ...
    const diasGrafico = 7;
    let labelsGrafico = [];
    let dadosGrafico = Array(diasGrafico).fill(0);
    const hojeZero = new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate());

    for (let i = diasGrafico - 1; i >= 0; i--) {
        const d = new Date(hojeZero);
        d.setDate(d.getDate() - i);
        labelsGrafico.push(d.toLocaleDateString('pt-PT', { day: '2-digit', month: '2-digit' }));
    }
    ...
    window.todasAsVendas.forEach(venda => {
        ...
        if (diffDias >= 0 && diffDias < diasGrafico) {
            const indexGrafico = (diasGrafico - 1) - diffDias;
            dadosGrafico[indexGrafico] += Number(venda.precoMZN) || 0;
        }
    });
    ...
    if(filtroServico === 'Todos') {
        custosTotais = custoPrime + custoNetflix;
    ...
    if (ctx) {
        if (window.meuGraficoVendas) {
            window.meuGraficoVendas.data.labels = labelsGrafico;
            window.meuGraficoVendas.data.datasets[0].data = dadosGrafico;
            window.meuGraficoVendas.update();
        } else {
            window.meuGraficoVendas = new Chart(ctx, {
                type: 'line',
                data: {
        ...
        if (passaPesquisa && passaServico && passaStatus) {
            ...
            if(window.baseDeDadosContas[venda.servico]) {
                btnPdfHtml = `<div style="display:flex; flex-wrap:wrap; gap:8px; justify-content: flex-end;">
                    ...
                    <button class="btn-icon" style="color: #25D366; border-color: rgba(37, 211, 102, 0.3);" onclick="enviarWhatsAppByID('${venda.id}')" title="Enviar Dados Iniciais"><i data-lucide="message-circle" size="14"></i> WA</button>
...
window.enviarWhatsApp = function(venda) {
    ...
    const texto = `Olá ${venda.nomeCliente}! Obrigado pela preferência.
...
window.gerarPDF = async function(venda) {
    ...
    document.getElementById('pdfPinPerfil').innerText = pinExato;
        ...
        <div class="grid-brands" style="margin-bottom: 24px;">
            <div class="brand-card netflix" onclick="mostrarFormularioConfiguracao('Netflix')">
                ...
                <div style="display: flex; justify-content: space-between; width: 100%; color: var(--text-muted); font-size: 14px;">
                    <span>Conta Registada</span><span style="color: white; font-weight: 600;">Configurar &rarr;</span>
                </div>
            </div>
...
window.mostrarFormularioConfiguracao = function(servico) {
    ...
    let perfisHtml = '<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 12px;">';
    
    dados.perfis.forEach((p, idx) => {
        perfisHtml += `
            <div style="background:var(--bg-elevated); padding:16px 12px; border-radius:12px; border:1px solid rgba(255,255,255,0.05); text-align:center;">
                <input type="text" id="cfg_${servico.replace(' ', '')}_nome_${idx}" value="${p.nome}" 
                    placeholder="Nome"
                    style="width:100%; text-align:center; font-size:12px; color:var(--text-muted); font-weight:600; margin-bottom:12px; text-transform:uppercase; letter-spacing:0.5px; background:transparent; border:none; border-bottom:1px solid rgba(255,255,255,0.1); padding-bottom:4px; outline:none;">
                <input type="text" id="cfg_${servico.replace(' ', '')}_pin_${idx}" value="${p.pin}" 
                    maxlength="${maxLength}" 
                    placeholder="${maxLength} d." 
                    oninput="this.value = this.value.replace(/[^0-9]/g, '')"
                    style="width:100%; text-align:center; letter-spacing:2px; font-weight:bold; background:rgba(0,0,0,0.3); border:1px solid rgba(255,255,255,0.1); border-radius:8px; padding:10px 8px; color:#fff; outline:none; transition:0.2s;">
            </div>
        `;
    });
    perfisHtml += '</div>';
    ...
    const html = `
        ...
        <div class="brand-card" style="cursor:default;">
            <h3 style="margin-top:0; border-bottom:1px solid rgba(255,255,255,0.1); padding-bottom:12px; margin-bottom:20px; font-size:18px;">Configurações: ${servico}</h3>
            
            <div class="input-group" style="margin-bottom: 12px;">
                <i data-lucide="mail" class="input-icon"></i>
                <input type="email" id="cfg_${servico.replace(' ', '')}_email" value="${dados.email}" placeholder="Email da Conta" style="background:var(--bg-elevated);">
            </div>
            
            <div class="input-group" style="margin-bottom: 24px;">
                <i data-lucide="lock" class="input-icon"></i>
                <input type="text" id="cfg_${servico.replace(' ', '')}_senha" value="${dados.senha}" placeholder="Palavra-passe" style="background:var(--bg-elevated);">
            </div>
